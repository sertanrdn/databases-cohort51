...
async function findEpisodesExercises(client) {
    ...
    console.log(
      `The title of episode 2 in season 2 is ${episode2.title}`
    );
    ...
    const lighthouseEpisodes = await client
        .db(dbName)
        .collection(collectionName)
        .find({ elements: { $all: ["CLIFF", "LIGHTHOUSE"] } })
        .toArray();
    const lighthouseTitles = lighthouseEpisodes.map(episode => episode.title);
    console.log(
      `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${lighthouseTitles.join(', ')}`
    );
}
...
async function updateEpisodeExercises(client) {
    ...
    const result1 = await client
        .db(dbName)
        .collection(collectionName)
        .updateOne({ episode: "S03E13" }, { $set: { title: "BLUE RIDGE FALLS" } });
    console.log(
